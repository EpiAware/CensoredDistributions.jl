...
# Right-truncate `dist` to `[minimum(dist), window]`. A non-positive window
# (the horizon has passed the anchor) gives an empty-support truncation,
# matching the andv `floatmin` guard on a zero denominator.
function _truncate_window(dist::UnivariateDistribution, window::Real)
    lower = minimum(dist)
    upper = max(window, lower)
