...
#
# Each block is gated by `ad_unit_runs` (defined in `runtests.jl`) so the
# per-backend CI jobs (#269) only run the unit tests relevant to the
# selected backend; with no backend selected every block runs.

if ad_unit_runs("core")
    @testset "_grad_p_a_series matches FiniteDifferences" begin
        ...
        # Restrict grid to (a, z) values where ∂P/∂a is large enough that
        # the finite-difference baseline is meaningful (i.e. away from
        # P-saturation). Includes k≪1 cases — the singular regime that
        # breaks naive implementations of the shape-parameter derivative.
        # Lower bound on a is set by the fdm probe radius (`central_fdm(7,
        # 1)` reaches into negative a if started below ~0.1).
        grid = [
...
if ad_unit_runs("Mooncake")
    @testset "_gamma_cdf passes Mooncake.TestUtils.test_rule" begin
        ...
        # `rrule!!` and the `frule` into an `frule!!`, so both interfaces
        # are registered (#270). For each mode, verifies (a) the rule is
        # actually invoked (is_primitive = true asserts this) and (b)
        # primal + derivative match Richardson-extrapolated finite
        # differences.
...
if ad_unit_runs("Enzyme")
    ...
    @testset "Enzyme gamma rule (issue #263)" begin
        ...
        # CensoredDistributionsEnzymeExt. With only
        # EnzymeSpecialFunctionsExt loaded, Enzyme mis-lowers `gamma` to
        # the `loggamma` known-op and returns `ψ(x)` instead of
        # `Γ(x) ψ(x)` — silently wrong by a factor of `Γ(x)` in both
        # modes. The analytical Gamma/Weibull `primarycensored_cdf` paths
        # call `gamma(k + 1)` / `gamma(1 + 1/k)` outside the `_gamma_cdf`
        # rule, so this gap corrupted the shape partial of the whole
        # pipeline.
        using ADTypes: AutoEnzyme, AutoForwardDiff
        using DifferentiationInterface: gradient
        using Enzyme: Enzyme
    ...
    @testset "Enzyme reverse on analytical scenarios (issue #263)" begin
        ...
        # corrupted. Uses the same runtime-activity + Duplicated settings
        # as the ADFixtures `Enzyme reverse` backend.
        using ADTypes: AutoEnzyme, AutoForwardDiff
        using DifferentiationInterface: gradient
        using Enzyme: Enzyme
        ...
        for ctor in (Gamma, Weibull)
            f = θ -> sum(
                x -> logpdf(
                    primary_censored(
                        ctor(θ[1], θ[2]), Uniform(0.0, 1.0)), x),
                obs)
            ...
            # ~1e-6. 1e-4 still catches the pre-fix shape partial, which
            # was wrong by a factor of `Γ(x)` (~150%).
...
if ad_unit_runs("core")
    @testset "_gamma_cdf rrule and _make_weibull_g zero-input guards" begin
        ...
        # scenario suite never hits (all gradient grids use strictly
        # positive x / t). Without this, the rrule's x <= 0 path and the
        # Weibull g's t <= 0 guard appear as uncovered defensive code in
        # patch coverage.
