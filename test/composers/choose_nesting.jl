# A `Choose` may be a CHILD of a composer (Sequential / Parallel / compose), not
# just a top node. The flat value-vector nesting machinery
# (`_child_nleaves` / `_child_logpdf` / `_child_rand!`) treats a nested Choose as a
...
@testitem "Choose reports a child leaf count" begin
    using CensoredDistributions, Distributions
    using CensoredDistributions: _child_nleaves

    # All alternatives share one leaf width, so a nested Choose occupies that
    # many flat value slots.
    s = choose(:a => Gamma(2.0, 1.0), :b => LogNormal(0.5, 0.4))
    ...
    bad = choose(:one => Gamma(2.0, 1.0),
        :two => Sequential(Gamma(1.0, 1.0), Gamma(2.0, 1.0)))
    @test_throws ArgumentError _child_nleaves(bad)
end

@testitem "Choose nests in a Parallel: logpdf and rand" begin
    using CensoredDistributions, Distributions, Random

    inner = choose(:a => Gamma(2.0, 1.0), :b => LogNormal(0.5, 0.4))
    par = Parallel(Gamma(3.0, 1.0), inner)
    # Two leaves: the plain branch and the Choose child (its first alternative).
    @test length(par) == 2

    x = [1.5, 2.3]
    # The flat value-vector logpdf scores the Choose child as its first
    # alternative (the data-free default).
    manual = logpdf(Gamma(3.0, 1.0), x[1]) + logpdf(Gamma(2.0, 1.0), x[2])
    @test logpdf(par, x) ≈ manual

    # rand fills both slots; the Choose slot is its first alternative's draw, so
...
@testitem "Choose nests in a Sequential: logpdf and rand" begin
    using CensoredDistributions, Distributions, Random
...
@testitem "Choose nests inside a compose front-end" begin
    using CensoredDistributions, Distributions, Random

    inner = choose(:a => Gamma(2.0, 1.0), :b => LogNormal(0.5, 0.4))
    # A Choose as a compose component nests as a Parallel branch.
