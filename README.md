...
**AD backend support** (see `test/ad/runtests.jl` for the test matrix)
...
ForwardDiff and ReverseDiff (tape) work on every primary-censored scenario via the analytical rrule and Dual extensions added in #250; `IntervalCensored Gamma arbitrary` still hits the `gamma_inc` `Dual` dispatch gap ([#217](https://github.com/EpiAware/CensoredDistributions.jl/issues/217)) because it routes through `Distributions.cdf(Gamma)` not the package's `_gamma_cdf`. ReverseDiff (tape) regressed on `PrimaryCensored LogNormal+Uniform numerical` after #230 removed the try/catch in `primarycensored_cdf`. Mooncake's plain `DifferentiationInterface.gradient` works on every scenario, but `DifferentiationInterfaceTest.test_differentiation` errors on the primary-censored Gamma/Weibull paths (DIT-Mooncake interaction, tracked in [#225](https://github.com/EpiAware/CensoredDistributions.jl/issues/225)). Enzyme forward/reverse fail on every scenario ([#225](https://github.com/EpiAware/CensoredDistributions.jl/issues/225)).
