...
# Heavy tutorials: a live MCMC fit and a multi-backend AD benchmark. Each runs
# once in a fresh subprocess so native/memory state cannot accumulate across the
# long-lived Documenter process.
