...
# ╔═╡ 38c7ee96-488a-4e9e-911c-7dd9bf409976
md"We first define then number of samples we would like."

# ╔═╡ 28bcd612-19f6-4e25-b6df-cb43df4f2a73
n = 2000

# ╔═╡ baef24c7-1515-403a-aeb8-b66ab729852b
md"Then we define the true parameters of the lognormal distribution."
...
md"Finally, we generate varying pwindow, swindow, and obs_time lengths to create realistic heterogeneous observation scenarios"
...
Instead of taking this approach we could have sampled from the distributions directly as we do in the getting started."
...
# ╔═╡ 70296e61-eb68-4ddf-987b-5751c9e2ba24
...
# ╔═╡ d8c17045-0b51-47f3-a49d-a1ee9c2ecf9d
fixed_model = fix(double_cens_model, (mu = meanlog, sigma = sdlog))

# ╔═╡ 7d8fa882-7f41-4dd8-924e-d152bd1e49e0
samples = sample(fixed_model, Prior(), n; progress = false)

# Extract the generated samples

# ╔═╡ ea37b637-9c7f-4de0-957d-bddbdd21048b
delays = [samples[i].y for i in 1:n]
...
Now we'll fit the full model that accounts for censoring using the same model structure we used to generate the data. This approach directly uses the package's distribution interface, which automatically handles:
